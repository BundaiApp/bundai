...
export default function KanjiDetail({ navigation, route }) {
  ...
  async function addCard({ kanjiName, kanji, hiragana, on, meanings, meaning, quizAnswers }) {
    await addFlashCard({
      variables: {
        userId: auth.userId,
        kanjiName: isWord ? kanji : kanjiName,
        hiragana: isKana ? '' : isWord ? hiragana : on[0],
        meanings: isWord ? [meaning] : meanings, // this needs to be changed
        quizAnswers
      }
    })
  }
  ...
  function transformVerbMeaning(meaning) {
    // Convert to lowercase, remove "to " from the beginning,
    // remove parentheses and their contents, remove forward slashes,
